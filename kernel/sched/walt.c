	     ...
	     int event, u64 wallclock, u64 irqtime)
{
	...
	if (!p_is_curr_task) {
		 ...
		 * window is not rolled over(from cpu view) - just split up and account
...
#endif
	trace_walt_update_history(rq, p, runtime, avg, samples, event);
	return;
}
